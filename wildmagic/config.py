...
def _scoped_bool(purpose: str | None, suffix: str, default: bool) -> bool:
    value = _first_config_value(
        _scoped_keys(purpose, suffix),
        "1" if default else "0",
    ).lower()
    if value in _TRUE_VALUES | {"json"}:
        return True
...
def get_wild_magic_model() -> str:
    return (
        get_config_value("WILDMAGIC_WILD_MODEL")
        or get_config_value("WILDMAGIC_MODEL", DEFAULT_MODEL)
        or DEFAULT_MODEL
    )
...
def get_dialogue_model() -> str:
    return (
        get_config_value("WILDMAGIC_DIALOGUE_MODEL")
        or get_wild_magic_model()
    )


def get_trade_model() -> str:
    return (
        get_config_value("WILDMAGIC_TRADE_MODEL")
        or get_dialogue_model()
    )


def get_town_model() -> str:
    return (
        get_config_value("WILDMAGIC_TOWN_MODEL")
        or get_wild_magic_model()
    )
...
def get_wild_magic_provider() -> str:
    return (
        get_config_value("WILDMAGIC_PROVIDER", DEFAULT_PROVIDER)
        or DEFAULT_PROVIDER
    ).lower()
...
def get_dialogue_provider() -> str:
    return (
        get_config_value("WILDMAGIC_DIALOGUE_PROVIDER")
        or get_wild_magic_provider()
    ).lower()


def get_trade_provider() -> str:
    return (
        get_config_value("WILDMAGIC_TRADE_PROVIDER")
        or get_dialogue_provider()
    ).lower()


def get_town_provider() -> str:
    return (
        get_config_value("WILDMAGIC_TOWN_PROVIDER")
        or get_wild_magic_provider()
    ).lower()
...
def ollama_host(purpose: str | None = None) -> str:
    ...
    return _first_config_value(keys, DEFAULT_OLLAMA_HOST)
...
def ollama_trade_temperature() -> float:
    value = get_config_value("WILDMAGIC_TRADE_TEMPERATURE")
    if value is None:
...
def ollama_trade_num_predict() -> int:
    value = get_config_value("WILDMAGIC_TRADE_NUM_PREDICT")
    if value is None:
        return _int_value("WILDMAGIC_DIALOGUE_NUM_PREDICT", 320, 32, 1024)
