...
PROP_TEMPLATES = {
    ...
    "broken_puppet_stage": PropTemplate("broken_puppet_stage", "U", "broken puppet stage", "A collapsible puppet theater, ribs snapped, its painted curtain still impossibly bright.", True, {"wood", "cloth", "flammable", "broken"}),
    ...
    # ── Old Traditions (buried strata of pre-charter magic) ──────────────────
    "pile_of_skulls": PropTemplate("pile_of_skulls", "%", "tally of skulls", "Craniums stacked with ceremonial care, each crown marked in faded ochre. A bone-singer's congregation.", False, {"bone", "ritual", "death"}),
    "open_sarcophagus": PropTemplate("open_sarcophagus", "[", "open sarcophagus", "The heavy lid has been shoved aside from within, unhurriedly. Whoever left did so on their own schedule.", True, {"stone", "death"}),
    "mummified_remains": PropTemplate("mummified_remains", "m", "mummified remains", "Wrapped in linen that was festival-bright once, clutching nothing. The knots are a love letter in an old code.", False, {"flesh", "dry", "flammable", "death"}),
    ...
    "death_mask": PropTemplate("death_mask", "d", "death mask", "A plaster cast of a face smiling very slightly, garlanded with dried flowers that kept their color.", False, {"stone", "death"}),
    ...
    "bone_throne": PropTemplate("bone_throne", "K", "bone throne", "A chair assembled from large bones, mortared and polished smooth by use. Whoever sat here was visited often.", True, {"bone", "death", "ritual"}),
    "sealed_burial_urn": PropTemplate("sealed_burial_urn", "u", "sealed burial urn", "A terracotta urn wax-sealed at the top. It rattles when tilted, agreeably.", False, {"stone", "death", "ash"}),
    "bone_chime": PropTemplate("bone_chime", "}", "bone chime", "Strings of carved finger-bones that ring softly in air that does not move.", False, {"bone", "magic", "music"}),
    "singing_stones": PropTemplate("singing_stones", "n", "singing stones", "A semicircle of split stones, each humming a different held note. Together they are almost a chord.", True, {"stone", "music", "magic"}),
    "crystal_garden": PropTemplate("crystal_garden", "*", "crystal garden", "Quartz blooms planted in cultivated rows. Someone came here, regularly, to water light.", False, {"crystal", "magic", "light"}),
    "blood_tide_basin": PropTemplate("blood_tide_basin", "u", "blood-tide basin", "A shell-lined basin that fills and ebbs on its own tide, miles from any sea.", False, {"stone", "blood", "water", "ritual", "magic"}),
    "echo_jar": PropTemplate("echo_jar", "j", "echo jar", "A wax-sealed clay jar. Held to the ear, it is still having a quiet argument in two voices.", False, {"stone", "music", "lore", "magic"}),
    "festival_mask": PropTemplate("festival_mask", "f", "festival mask", "A lacquered dance mask trailing too many ribbons, hung at eye level. The eyeholes are warm.", False, {"wood", "cloth", "magic", "music"}),
    "ancestor_drum": PropTemplate("ancestor_drum", "o", "ancestor drum", "A broad drum whose beat carries on for a moment after you stop touching it.", False, {"wood", "leather", "music", "magic"}),
    "wind_organ": PropTemplate("wind_organ", "|", "wind organ", "Pipes of hollow reed and bone fitted into a wall crack, played slowly by the draft.", True, {"bone", "wood", "music"}),
    "painted_prayer_stones": PropTemplate("painted_prayer_stones", ";", "painted prayer stones", "River stones painted in patterns of dots and suns, stacked by steady hands.", False, {"stone", "holy"}),
    ...
    "requisition_ledger": PropTemplate("requisition_ledger", "=", "requisition ledger", "A ledger of goods seized and compensation scheduled. Every line is initialed. Every line.", False, {"paper", "lore", "empire"}),
    "regulation_lantern": PropTemplate("regulation_lantern", "!", "regulation lantern", "An imperial road lantern of regulation height, burning a precise, well-behaved blue.", True, {"metal", "light", "empire"}),

    # ── Natural & Overgrown ───────────────────────────────────────────────────
    "bioluminescent_mushroom": PropTemplate("bioluminescent_mushroom", "p", "bioluminescent mushroom", "Casting a cool green glow that brightens, just slightly, when you stand close.", False, {"plant", "fungus", "light"}),
    ...
    "crystal_formation": PropTemplate("crystal_formation", "*", "crystal formation", "Mineral spars jutting from the wall like teeth, each one a different color of dusk.", True, {"crystal", "stone", "sharp"}),
    ...
    "warning_sign": PropTemplate("warning_sign", "+", "warning sign", "A wooden board painted in three languages. The third is just a picture of someone running.", True, {"wood", "flammable", "lore"}),
    ...
    "caravan_hitching_post": PropTemplate("caravan_hitching_post", "W", "caravan hitching post", "A worn post ringed with knots tied in a dozen different traditions of rope-craft.", True, {"wood", "rope"}),
    ...
    "sacrificial_pit": PropTemplate("sacrificial_pit", "v", "offering pit", "A stone-lined hollow stained the deep brown of old gifts. Whatever was given here was given willingly.", False, {"stone", "blood", "ritual"}),
    ...
    "painting_of_somewhere_else": PropTemplate("painting_of_somewhere_else", "P", "painting of somewhere else", "A canvas of a harbor city you don't recognize. The lanterns in it are lit, and one has just gone out.", False, {"cloth", "magic", "lore"}),
...
PROP_CATEGORIES: dict[str, list[str]] = {
    ...
    "ruined": [
        "rotting_bookshelf", "overturned_cart", "broken_puppet_stage", "faded_tapestry",
    ...
    "traditions": [
        "pile_of_skulls", "open_sarcophagus", "mummified_remains",
        "funeral_pyre_remnants", "death_mask", "inscribed_gravestone",
        "bone_throne", "sealed_burial_urn", "bone_chime", "singing_stones",
        "crystal_garden", "blood_tide_basin", "echo_jar", "festival_mask",
        "ancestor_drum", "wind_organ", "painted_prayer_stones",
    ],
    "imperial": [
        "posted_notice", "charter_waystone", "survey_marker",
        "confiscation_crate", "requisition_ledger", "regulation_lantern",
    ],
    "natural": [
    ...
    "infrastructure": [
        ...
        "mine_support_beam", "caravan_hitching_post", "old_well", "cistern", "iron_grill_floor",
    ...
    "furniture": [
        ...
        "broken_hourglass", "locked_chest", "painting_of_somewhere_else", "tattered_map",
