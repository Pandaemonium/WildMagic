...
`make_town_provider` use `wildmagic.config` to select the active backend.
Ollama-backed providers carry a purpose label (`wild`, `dialogue`, `trade`, `town`)
so configuration can route urgent and background requests to different Ollama hosts.
...
`normalize_ollama_url`, and `fetch_ollama_models`. Configuration helpers are
re-exported from `wildmagic.config` for compatibility.

### `wildmagic/config.py`
The single configuration boundary. Loads the project `.env` without overriding
shell values, owns defaults and typed parsing, resolves provider/model fallback
chains, purpose-scoped Ollama routing and request options, and persists in-game
configuration changes back to `.env`.
