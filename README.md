...
cp .env.example .env
```
The `.env` file is the canonical persisted configuration. The in-game configuration menu writes changes back to it. Existing shell environment variables override `.env` values when the process starts.
